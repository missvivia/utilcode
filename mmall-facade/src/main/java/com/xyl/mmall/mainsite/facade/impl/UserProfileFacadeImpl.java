...
public class UserProfileFacadeImpl implements UserProfileFacade {
	...
	public BaseJsonVO saveUserProfile(long userId, MainSiteUserVO userVO) {
		...
		if (!isValidUserProfile(userVO)) {
			result.setCode(ResponseCode.RES_ERROR);
			result.setMessage("请完整填写基本信息！");
			return result;
		}
	 ...
	 * @param userVO
	 *            用户基本信息
	 * @return 如果必填信息完整，返回true；否则返回false
	 */
	private boolean isValidUserProfile(MainSiteUserVO userVO) {
		return StringUtils.isNotBlank(userVO.getNickname());
	}
