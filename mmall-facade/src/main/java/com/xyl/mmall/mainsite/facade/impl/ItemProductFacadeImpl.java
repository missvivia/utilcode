 ...
 */
@Facade
public class ItemProductFacadeImpl implements ItemProductFacade {

	private static Logger logger = Logger.getLogger(ItemProductFacadeImpl.class);
	...
	public List<ProductSKUMainSiteVO> getProudctByParameters(BasePageParamVO<ProductSKUMainSiteVO> basePageParamVO,
			...
			if (contentVO != null) {
				...
				if (contentVO.getLevel() == CategoryContentLevel.LEVEL_THIRD.getIntValue()) {
					if (CollectionUtils.isEmpty(contentVO.getCategoryNormalVOs())
							|| CollectionUtils.isEmpty(contentVO.getSendDistrictDTOs())) {
						return null;
					}
		...
		if (!CollectionUtils.isEmpty(paramVO.getList())) {
			...
			for (ProductSKUDTO skuDTO : paramVO.getList()) {
				ProductSKUMainSiteVO productSKUMainsiteVO = new ProductSKUMainSiteVO(skuDTO);
...
//			}
			...
			for (ProductSKUMainSiteVO p : retList) {
				...
				// 限购配置
				if (p.getIsSKULimited() == 1) {
					ProductSKULimitConfigDTO limitConfigDTO = 
							productSKULimitService.getProductSKULimitConfigBySkuId(p.getSkuId());
					if (limitConfigDTO != null) {
						p.setSkuLimitConfigVO(new ProductSKULimitConfigVO(limitConfigDTO));
//						p.setLimitBuyNum(productSKULimitService.getTotalBuyNum(p.getSkuId(), uid));
					}
				}
...
//					cartStk = 0;
	...
	public ProductSKUMainSiteVO getProductSKUVO(ProductSKUDTO skuDTO) {
		...
		// 商品限购信息
//		long uid = SecurityContextUtils.getUserId();
		if (productSKUVO.getIsSKULimited() == 1) {
			ProductSKULimitConfigDTO limitConfigDTO = 
					productSKULimitService.getProductSKULimitConfigBySkuId(productSKUVO.getSkuId());
			if (limitConfigDTO != null) {
				productSKUVO.setSkuLimitConfigVO(new ProductSKULimitConfigVO(limitConfigDTO));
//				productSKUVO.setLimitBuyNum(
//						productSKULimitService.getTotalBuyNum(productSKUVO.getSkuId(), uid));
			}
		}
		// 库存
	...
	private ProductSKUMainSiteVO setSPUInfo(ProductSKUMainSiteVO productSKUBackendVO) {
		...
		productSKUBackendVO.setCategoryFullName(
				categoryService.getFullCategoryNormalName(productSKUBackendVO.getCategoryNormalId()));
	...
	public List<JSONObject> getBrands(List<Long> categoryNormalIds, String searchValue, long areaId) {
		...
		// 先获取品牌id
		List<Long> brandIds = 
				itemProductService.getBrandIdsByCategoryIds(categoryNormalIds, searchValue, businessIds);
