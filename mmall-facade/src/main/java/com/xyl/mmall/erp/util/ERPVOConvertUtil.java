package com.xyl.mmall.erp.util;
...
import com.xyl.mmall.erp.vo.OrderCartItemErpVO;
import com.xyl.mmall.erp.vo.OrderSkuErpVO;
...
import com.xyl.mmall.mainsite.vo.order.OrderFormBasicVO;
...
/**
 * ERPVOConvertUtil.java created by yydx811 at 2015年11月2日 下午4:59:11
 * ErpVO对象的转换工具
 *
 * @author yydx811
 */
public class ERPVOConvertUtil {
...
//			String title = cartVOList.get(0).getOrderSkuList().get(0).getSkuSPDTO().getPoName();
//
//			OrderNoActionVO noActionVO = new OrderNoActionVO();
//			noActionVO.setTitle(title);
...
//	}
	 ...
	 * @return
	 */
	private static List<OrderCartItemErpVO> convertToOrderCartItemVOList(Map<Long, OrderCartItemDTO> skuIdAndCartDTOMap,
		...
		List<OrderCartItemErpVO> cartVOList = convertToOrderCartItemVOList(cartDTOList, isSnapShot);
		return cartVOList;
	}
...
//	}
	 ...
	 * @return
	 */
	public static List<OrderCartItemErpVO> convertToOrderCartItemVOList(List<? extends OrderCartItemDTO> dtoList,
			Boolean isSnapShot) {
		List<OrderCartItemErpVO> voList = new ArrayList<>();
		if (CollectionUtil.isEmptyOfCollection(dtoList))
			return voList;
	 ...
	 * @return
	 */
	private static OrderCartItemErpVO convertToOrderCartItemVO(OrderCartItemDTO cartDTO, Boolean isSnapShot) {
		boolean isFilterEnum = false;
		OrderCartItemErpVO vo = ReflectUtil.convertObj(OrderCartItemErpVO.class, cartDTO, isFilterEnum);
		...
		for (OrderSkuErpVO orderSkuVO : vo.getOrderSkuList()) {
	 ...
	 * @return
	 */
	public static List<OrderSkuErpVO> convertToOrderSkuVOList(List<? extends OrderSkuDTO> dtoList, Boolean isSnapShot) {
		List<OrderSkuErpVO> voList = new ArrayList<>();
		if (CollectionUtil.isEmptyOfCollection(dtoList))
			return voList;
	...
	/**
	 * 将OrderSkuDTO转换成OrderSkuErpVO
	 * 
	 * @param orderSkuDTO
	 * @param isSnapShot
	 * @return
	 */
	private static OrderSkuErpVO convertToOrderSkuVO(OrderSkuDTO orderSkuDTO, Boolean isSnapShot) {
		boolean isFilterEnum = false;
		OrderSkuErpVO vo = ReflectUtil.convertObj(OrderSkuErpVO.class, orderSkuDTO, isFilterEnum);
		vo.setSkuSPVO(convertToSkuSPVO(orderSkuDTO.getSkuSPDTO(), isSnapShot, orderSkuDTO));
	 ...
	 * @return
	 */
	private static SkuSPDTO convertToSkuSPVO(SkuSPDTO skuSPDTO, Boolean isSnapShot, OrderSkuDTO dto) {
		if (skuSPDTO != null && isSnapShot == Boolean.TRUE) {
			skuSPDTO.setLinkUrl(UrlBaseUtil.buildFullPathUrl(skuSPDTO.getLinkUrl()));
			skuSPDTO.setSnapShotUrl(
					UrlBaseUtil.buildSkuSnapShotUrl(dto.getSkuId(), dto.getOrderId(), dto.getUserId()));
		}
		if (skuSPDTO == null) {
			skuSPDTO = new SkuSPDTO();
		}
		return skuSPDTO;
	}
