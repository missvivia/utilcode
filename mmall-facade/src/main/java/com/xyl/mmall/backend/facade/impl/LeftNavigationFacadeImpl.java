 ...
 *
 */
@Facade("backendLeftNavigationFacade")
public class LeftNavigationFacadeImpl implements LeftNavigationFacade {
	...
	public List<LeftNavItemVO> getLeftNavigationMenu(long userId) {
		...
		List<PermissionDTO> totalPermissionList = permissionService.findDealerPermissionsByDealerId(userId);
		...
		if (!CollectionUtils.isEmpty(permissionList)) {
			...
			for (PermissionDTO permissionDTO : permissionList) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Dealer (id = {}) has left nav item:{}.", userId, permissionDTO.getName());
				}
