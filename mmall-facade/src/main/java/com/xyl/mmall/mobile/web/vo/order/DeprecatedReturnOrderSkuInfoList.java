 ...
 */
@Deprecated
public class DeprecatedReturnOrderSkuInfoList implements Serializable {
	...
	//  --------------------------------
		 ...
		 * @param couponOrder
		 */
		public void fillWithOrderSku(
				OrderSkuDTO ordSkuDTO, 
				DeprecatedOrderReturnJudgement orj, 
				DeprecatedOrderSkuReturnJudgement osrj, 
			...
			this.salePrice = ordSkuDTO.getOriRPrice();
			...
			// 3. 退货审核
			if(null == orj) {
				this.status = DeprecatedOrderReturnJudgement.NULL;
				return;
			}
			if(!orj.isCanReturn()) {
				this.status = orj;
			} else {
				this.status = (null == osrj) ? DeprecatedOrderSkuReturnJudgement.NULL : osrj;
			}
		}
		...
		public void fillWithRetOrderSku(
				DeprecatedReturnOrderSkuDTO retOrdSkuDTO, 
				DeprecatedOrderReturnJudgement orj, 
				DeprecatedOrderSkuReturnJudgement osrj, 
				PromotionDTO promotion, 
				Coupon coupon
			...
			fillWithOrderSku(ordSkuDTO, orj, osrj, promotion, coupon);
			// adjust quantity&sum
			this.quantity = (int) retOrdSkuDTO.getReturnCount();
			if(null != this.salePrice) {
				this.sum = this.salePrice.multiply(new BigDecimal(retOrdSkuDTO.getReturnCount()));
