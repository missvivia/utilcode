<#assign pageName="storage-logistics"/>
<!doctype html>
<html lang="en">
<head>
...
<@crumbs parent={"txt":"物流仓库管理","url":'#'} sub={"txt":"物流统计"}/>
...
<div class="row">
  <div class="col-sm-12">
    <div class="m-card">
      ...
      <div class="card_c">
        <form method="get" class="form-horizontal" id="search-form" action="/storage/logistics/export">
          <div class="form-group">
            <label class="col-sm-1 control-label">报表类型：</label>
            <div class="col-sm-3">
              <select name="type" class="form-control" data-type="number" >
                <#list typeListOfLog as key>
                  <option value="${key?substring(2,3)}">${key}</option>
                </#list>
              </select>
            </div>
          </div>
          <div id="warehouse-sel" class="form-group <#if typeListOfLog?has_content && typeListOfLog[0] =="报表1-全国物流概况">f-dn</#if>">
          ...
          <div class="form-group">
            ...
            <div id="export-ct" class="col-sm-1 <#if typeListOfLog?has_content && (typeListOfLog[0] !="报表5-未签收明细表" || typeListOfLog[0] !="报表7-未返回单量明细")>f-dn</#if>">
              <input type="submit" class="form-control btn btn-primary" value="导出"/>
            </div>
          </div>
        </form>
        <div id="m-logisticslist"></div>
      </div>
    </div>
  </div>
</div>
...
<script src="${jspro}page/storage/logistics.js?v=1.0.0.0"></script>
