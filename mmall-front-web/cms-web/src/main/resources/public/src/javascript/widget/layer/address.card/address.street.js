...
define(['base/klass'
	...
	pro.__reset = function(options) {
		...
		this.__url = options.url || "/location/";
	...
	pro.__resetTab = function(index){
		...
		if(index==0){
			...
			if(_cityCache[this.__data.provinceId]){
				if(ut._$indexOf(_zhiXiaCities,this.__data.provinceId)!=-1){
					this.__data.cityId = "-" + this.__data.provinceId + "01";
					this.__resetTab(2); 
					return;
				}
				e2._$render(this.__box,_seed_data,{type:1,cityList:_cityCache[this.__data.provinceId],data:this.__data});
				return;
			}
			request(this.__url + 'city',{
				data:ut._$object2query({code:this.__data.provinceId}),
				onload:this.__cbGetCity._$bind(this,this.__data.provinceId),
				onerror:this.__cbGetCity._$bind(this,this.__data.provinceId),
				type:'json'
			});
		} else if(index==2){
			//  /src/javascript/widget/layer/address.card/mock/section.json
			if(_districtCache[this.__data.cityId]){
				e2._$render(this.__box,_seed_data,{type:2,sectionList:_districtCache[this.__data.cityId],data:this.__data})
			}else{
				request(this.__url + 'district',{
					data:ut._$object2query({code:this.__data.cityId}),
					onload:this.__cbGetSection._$bind(this,this.__data.cityId),
					onerror:this.__cbGetSection._$bind(this,this.__data.cityId),
					type:'json'
				});
			}
		} else if(index==3){
			// /src/javascript/widget/layer/address.card/mock/street.json
			if(_streetCache[this.__data.sectionId]){
				e2._$render(this.__box,_seed_data,{type:3,streetList:_streetCache[this.__data.sectionId],data:this.__data});
			}else{
				request(this.__url + 'street',{
					data:ut._$object2query({code:this.__data.sectionId}),
					onload:this.__cbGetStreet._$bind(this,this.__data.sectionId),
					onerror:this.__cbGetStreet._$bind(this,this.__data.sectionId),
					type:'json'
				});
			}
		} 
	...
	pro.__cbGetCity = function(provinceId,result){
		if(result.code==200){
			_cityCache[provinceId] = result.result.list;
	...
	pro.__cbGetSection = function(cityId,result){
		if(result.code==200){
			if(result.result.list.length==0){
				this._$dispatchEvent('onchange',this.__data,true);
				this._$hide();
			} else{
				_districtCache[cityId] = result.result.list;
	...
	pro.__cbGetStreet = function(districtId,result){
		if(result.code==200){
			_streetCache[districtId] = result.result.list;
