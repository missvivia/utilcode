...
public class AreaFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
		...
		if (!AreaUtils.validateSign(httpServletRequest)) {
			...
			long cityCode = -5001, districtCode = 500112l;// 默认重庆 渝北区,不允许切换
			String cityName = "重庆市", districtName = "渝北区";
			...
			if (uid > 0l) {
				...
				if (address != null) {
					...
					districtCode = address.getSectionId();
					districtName = address.getSection();
				}
			}

			AreaUtils.setAreaCookie((HttpServletResponse) response, districtCode);
			request.setAttribute(AreaUtils.COOKIE_NAME_AREA, districtCode);
		}
