 ...
 *	pagination: '.pagination', // 显示位置
 *  loop: true // 循环
 *})
 * ```
 * 
 * ppfyang(hzyang_fan@corp.netease.com)
 * ppRevise: 
 * 具体位置 可搜索"新加"即可
 * reInit: 新增方法 更新滚动条和 滚动区域     param: 无 ; return: 无
 * allowMomentumBounce : 新增属性   关闭回弹标记    防止惯性到底回弹过程中 用户点击回顶按钮  失效bug
 * scrollCallback: 新增方法 touchEnd callback   param: Swiper对象; return: 无
 * swiperCallback: 新增方法 切页结束 回调页码函数     param: 切页 页码 (从0开始); return: 无
 * position .max  .end:  新增属性   用于判断回弹位置
 * 
 * ps: 使用时  请压缩
 * 
*/
...
var Swiper = function (selector, params) {
    ...
    var defaults = {
        ...
        //scrollCallback:新加属性 滑动结束 回调函数
        scrollCallback: false,
        //swiperCallback:新加属性 切页结束 回调页码函数
        swiperCallback : false
    ...
    _this.allowMomentumBounce = true;
    function onTouchStart(event) {
        ...
        _this.allowMomentumBounce = false;
    ...
    function onTouchEnd(event) {
        ...
        var maxPosition = maxWrapperPosition();
       	_this.positions.maxPosition = maxPosition;
        var crossTop = !!(_this.positions.current > 0);
        var crossBottom = !!(_this.positions.current < -maxPosition);
        ...
        //Free Mode
        if (params.freeMode&&!crossTop&&!crossBottom) {
            if ( params.freeModeFluid ) {
                ...
                if (params.momentumBounce && doBounce) {
                    _this.wrapperTransitionEnd(function(){
                        if (!_this.allowMomentumBounce) return;
                ...
                //新加属性  比较惯性滚动后的具体位置
                _this.positions.end = newPosition;
            }
    ...
    _this.updatePagination = function(position) {
        ...
        if (params.paginationAsRange) {
            ...
            //新加方法  回调当前切页 页码 
            if (!!params.swiperCallback)
				params.swiperCallback(_this.activeIndex);
        }
