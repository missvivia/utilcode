...
/**
 * 主站用户相关服务，为安全验证提供用户信息和权限信息等。
 * 
 * @author lihui
 *
 */
@Service("applicationAuthUserService")
public class MainSiteAuthUserService extends SimpleUserService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(MainSiteAuthUserService.class);
...
//			// 获取用户的昵称
//			nickName = ursAuthUserInfoService.getNicknameFromURS(userName);
//		}
		...
		if (address != null && address.getSectionId() > 0l) {
			...
			if (subject != null) {
				...
				AreaUtils.setAreaCookie(response, address.getSectionId());
			}
		}
		return buildMainSiteUser(mainSiteUserVO);
	}
