...
public final class AreaUtils {
	...
	public static boolean validateSign(HttpServletRequest request) {
		...
		if (cookieArray != null && cookieArray.length >= 3) {
			String areaCode = cookieArray[0];
			String createTime = cookieArray[1];
	...
	private static String createSign(String areaCode, String creatTime) {
		String input = areaCode + OWNER_KEY;
		byte[] sign = DigestUtils.sha1(input.getBytes(), creatTime.getBytes(), 10);
		return Hex.encodeToString(sign);
	}
