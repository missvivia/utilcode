...
public class CartController {
	...
	@BILog(action = "page", type = "cartPage")
	@CheckFormToken(isCheckRepeat = false)
	@RequestMapping(value = "/cartlist", method = RequestMethod.GET)
	public String getCartPage(Model model) {
		long userId = SecurityContextUtils.getUserId();
		CartVO cartVO = cartFacade.getCartInfo(userId, getProviceId(), null, PlatformType.PC, false);
	...
	public @ResponseBody BaseJsonVO listmini(Model model,
		...
		BaseJsonVO retObj = new BaseJsonVO(cartFacade.getCartInfo(userid, getProviceId(), selectIds, PlatformType.PC,
				true));
	 ...
	 * @return
	 */
	@BILog(action = "click", type = "addToCart")
	...
	public @ResponseBody BaseJsonVO updateCartamount(Model model, @RequestBody InputParam obj) {
		long userid = SecurityContextUtils.getUserId();
		int result = cartFacade.updateCartAmount(NkvConstant.NKV_CART_VALID, userid, getProviceId(),
				obj.getCartItemDTOs());
		BaseJsonVO retObj = new BaseJsonVO();
		if (result > 0) {
			retObj.setCode(ErrorCode.SUCCESS);
	...
	public @ResponseBody BaseJsonVO deleteProduct(Model model, @RequestBody InputParam obj) {
		long userId = SecurityContextUtils.getUserId();
		boolean result = cartFacade.deleteCartItems(userId, getProviceId(), obj.ids);
		BaseJsonVO retObj = new BaseJsonVO();
		if (result) {
			retObj.setCode(ErrorCode.SUCCESS);
		} else {
	...
	public @ResponseBody BaseJsonVO deleteCart(Model model) {
		long userid = SecurityContextUtils.getUserId();
		BaseJsonVO retObj = new BaseJsonVO();
		boolean result = cartFacade.deleteCart(userid, getProviceId());
		if (result) {
			retObj.setCode(ErrorCode.SUCCESS);
		} else {
			retObj.setCode(201);
	...
	public String getcart(Model model) {
		long userId = SecurityContextUtils.getUserId();
		CartVO cartVO = cartFacade.getCart(userId, getProviceId(), null, PlatformType.PC);
		// CartVO cartVO = cartFacade.getCartInfo(userId, getProviceId(), null,
		// PlatformType.PC, true);
		model.addAttribute("cartVO", cartVO);
		model.addAttribute("cartEndTime", cartFacade.getCartLeftTime(userId, getProviceId()));
		if ((cartVO.getPoList() != null && cartVO.getPoList().size() > 0)
				|| (cartVO.getActivations() != null && cartVO.getActivations().size() > 0)) {
			return "pages/cart/cartlist";
		} else {
	...
	public @ResponseBody BaseJsonVO list(Model model) {
		long userid = SecurityContextUtils.getUserId();
		BaseJsonVO retObj = new BaseJsonVO(cartFacade.getCart(userid, getProviceId(), null, PlatformType.PC));
		retObj.setCode(ErrorCode.SUCCESS);
		return retObj;
	}
	...
	public @ResponseBody BaseJsonVO rebuy(Model model, @RequestBody InputParam obj) {
		long userid = SecurityContextUtils.getUserId();
		int ret = cartFacade.rebuyAndReturnCode(userid, getProviceId(), obj.skuId, 1);
		...
		CartVO cartvo = cartFacade.getCart(userid, getProviceId(), obj.selectedIds, PlatformType.PC);
	...
	public @ResponseBody BaseJsonVO select(Model model, @RequestBody InputParam obj) {
		long userid = SecurityContextUtils.getUserId();
		BaseJsonVO retObj = new BaseJsonVO(cartFacade.getCart(userid, getProviceId(), obj.selectedIds, PlatformType.PC));
		retObj.setCode(ErrorCode.SUCCESS);
		return retObj;
	...
	public @ResponseBody BaseJsonVO delete(Model model, @RequestBody InputParam obj) {
		long userId = SecurityContextUtils.getUserId();

		cartFacade.deleteCartItem(userId, getProviceId(), obj.ids);
		BaseJsonVO retObj = new BaseJsonVO();
	...
	public @ResponseBody BaseJsonVO deletemini(Model model, @RequestBody InputParam obj) {
		long userId = SecurityContextUtils.getUserId();

		cartFacade.deleteCartItem(userId, getProviceId(), obj.ids);
		BaseJsonVO retObj = new BaseJsonVO(cartFacade.getCartMini(userId, getProviceId(), null));
		retObj.setCode(ErrorCode.SUCCESS);
		return retObj;
	...
	public @ResponseBody BaseJsonVO resettime(Model model) {
		long userid = SecurityContextUtils.getUserId();
		BaseJsonVO retObj = new BaseJsonVO();
		retObj.setResult(cartFacade.resetTime(userid, this.getProviceId()) + CartService.CART_TIME);
		retObj.setCode(ErrorCode.SUCCESS);
		return retObj;
	...
	public @ResponseBody BaseJsonVO addUserToRemindWhenStorage(Model model, @RequestParam long skuId,
			@RequestParam int subscribed) {
		long userid = SecurityContextUtils.getUserId();
		BaseJsonVO retObj = new BaseJsonVO();
		boolean flag = false;
		if (subscribed == 1) {
			flag = cartFacade.addUserToRemindWhenStorage(userid, this.getProviceId(), skuId);
		} else if (subscribed == 0) {
			flag = cartFacade.removeUserRemind(userid, this.getProviceId(), skuId);
		} else {
	...
	public @ResponseBody BaseJsonVO userExistInRemind(Model model, @RequestParam long skuId) {
		long userid = SecurityContextUtils.getUserId();
		boolean flag = cartFacade.userExistInRemindStorage(userid, this.getProviceId(), skuId);
