...
NEJ.define([
  ...
  var SizePicker = BaseComponent.extend({
    ...
    data: {
      classMaps: {
        1: "u-sel u-sel-size",
        2: "u-sel u-sel-size u-sel-gray",
        3: "u-sel u-sel-size u-sel-dis"
      }
    },
    ...
    configSize: function(){
      ...
      if(total === chance+empty) setTimeout(this.$emit.bind(this,'empty',chance>0?5:4), 0)
    },
    init: function(){
      var data = this.data;
      if(data.skuId){
    ...
    select: function(size){
      ...
      var self = this;
      // @REMOVE
      this.checkSize(/*size.skuId,*/function(err){
        //如果type变为非1，则不选择
        // 出现错误我们也不管，到购物车再做提醒。
        if(size.type === 1 || err){
    ...
    // 同步库存
    checkSize: function(skuId, callback){
      if(typeof skuId === "function"){
        callback = skuId;
        skuId = null;
      }
      var data = this.data;
      this.$request( url.CHECK_SIZE, {
        data: {id: data.id, scheduleId: data.scheduleId, skuId: skuId},
        onload: function(json){
          ...
          if(total === chance+empty) this.$emit("empty", chance>0?5:4);
