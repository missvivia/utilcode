...
<body>
	...
	<div class="content-bg">
		...
		<div class="second-category wrap clearfix">
			<table class="filter" <#if !categoryContentList?? && !parameterList?? && !specificationList?? && !brandList?? && !contentVO??>style="display: none;"</#if>>
				<#if categoryContentList?? >
					<tr>
						<#list categoryContentList as items>
							<th width='7%'>${items.name}</th>
							<td>
								<div class='item-box clearfix'>
									<i class='action'>展开</i>
									<#if items.subCategoryContentList??>
										<#list items.subCategoryContentList as item1>
											 <a href="/list/${item1.id}">${item1.name}</a>
				...
				<!-- 二级分类 -->
				<#if contentVO?? >
					<tr <#if contentVO.level=3>style="display: none;"</#if>>
						<#if contentVO.name?? >
							<th width='7%'>${contentVO.name}</th>
						</#if>
						<td>
							<div class='item-box clearfix'>
								<i class='action'>展开</i>
								<#if contentVO.categoryNormalVOs??>
									<#list contentVO.categoryNormalVOs as item>
										<a href="/list/${contentVO.id}-${item.categoryId}">${item.categoryName}</a>
									</#list>
								</#if>
							</div>
						</td>
    ...
    <script type="text/javascript">
    	$(function () {
			...
			if (key1 >= 0 && key2 >= 0 && key3 >= 0) {
				// url格式为 xxx/100010-1000011,1000012
				categoryContentId = url.substring(key1 + 1, key2);
				categoryNormalId = "";
			} else if (key1 >= 0 && key2 >= 0 && key3 < 0) {
				// url格式为 xxx/100010-1000011
				categoryContentId = url.substring(key1 + 1, key2);
				categoryNormalId = url.substring(key2 + 1, urlLen);
			} else {
				// url格式为 xxx/100010
				categoryContentId = url.substring(key1 + 1, urlLen);
				categoryNormalId = "";
			}
			...
			function createLabel(_this) {
				...
				createBrandFilter(brandNames, title);
			}

			function createBrandFilter(brandNames, title) {
				//console.log(brandNames);
				if (brandNames != "") {
					var dom = "";
					dom += "<li>"+
								"<em>"+ title +"：</em>"+
								"<span>"+ brandNames +"</span>"+
								"<i>x</i>"+
							"</li>";
					$(".brand-fillter ul").empty();
					$(".brand-fillter ul").append(dom);
				} else {
					$(".brand-fillter ul").empty();
				}
			}
