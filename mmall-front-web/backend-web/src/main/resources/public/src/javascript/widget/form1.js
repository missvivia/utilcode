...
NEJ.define([
    ...
    _pro.__doInitFormCtrl = function(_form,_options){
            ...
            function(_parent){
            	if(_parent.tagName=='INPUT'){
	                ...
	                var _name = _e._$dataset(_parent,'name'),
	                    _value = parseInt(_e._$dataset(_parent,'value')),
        ...
        // init category
        if(window.flag != 1){
        	_options.categoryIds = [window.firstCategoryId, window.secondCategoryId, window.lowCategoryId];
        }
        
        var _list = _e._$getByClassName(_form,'j-cate');
        if (!!_list&&_list.length>0){
            this.__selector = _t1._$$CascadeSelector._$allocate({
            	keys:{id:'categoryId',text:'categoryName',list:'subCategoryList'},
                select:_list,
