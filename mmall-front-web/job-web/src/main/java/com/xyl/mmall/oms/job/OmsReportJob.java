 ...
 */
@Service
@JobPath("/oms/genReportSendOut")
public class OmsReportJob extends BaseJob {

	@Autowired
	private OmsReportFacade omsReportFacade;

	private static final Logger logger = LoggerFactory.getLogger(OmsReportJob.class);
	...
	public synchronized boolean execute(JobParam param) {
		...
		try {
			...
			omsReportFacade.processSendOUt(date);
 		} catch (Exception e) {
