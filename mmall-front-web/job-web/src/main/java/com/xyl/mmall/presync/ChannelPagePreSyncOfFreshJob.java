 ...
 */
@Service
@JobPath("/presync/channelpage/fresh")
public class ChannelPagePreSyncOfFreshJob extends BasePreSyncJob {
	...
	private static final Logger logger = LoggerFactory.getLogger(ChannelPagePreSyncOfFreshJob.class);
	...
	public boolean execute(JobParam param) {
		logger.info("===begin presync channel page fresh");
		List<AreaDTO> areaList = businessFacade.getAreaList();
		if (areaList == null || areaList.size() <= 0) {
			logger.error("cannot find area when presync channel fresh page");
			return false;
		}
		...
		logger.info("===end presync channel page fresh");
	...
	private boolean invokeByChannelType(int channelType, List<AreaDTO> areaList) {
		...
		for (AreaDTO areaDTO : areaList) {
			...
			if (!super.invokeUrl(url, 30)) {
				successFlag = false;
				logger.error("===presync channel page fresh for area:" + areaDTO.getId() + " error");
			}
		}
		return successFlag;
	}
