 ...
 */
@Service
@JobPath("/oms/genReportReturn")
public class OmsReportReturnJob extends BaseJob {

	@Autowired
	private OmsReportFacade omsReportFacade;

	private static final Logger logger = LoggerFactory.getLogger(OmsReportReturnJob.class);
	...
	public synchronized boolean execute(JobParam param) {
		...
		try {
			...
			omsReportFacade.processReturn(date);
 		} catch (Exception e) {
