...
import com.xyl.mmall.security.filter.NEAuthenticatingMainSiteTestingFilter;
...
import com.xyl.mmall.security.service.URSOAuthServiceInternalTestingImpl;
...
@Configuration
@Profile("performance")
@PropertySource(value = "classpath:/config/performance/mainSiteSecurity.properties")
@ConditionalOnClass(name = "com.xyl.mmall.service.MainSiteAuthUserService")
public class MainSiteSecurityConfig extends BaseSecurityConfig {
	...
	Filter authWebFilter() {
		NEAuthenticatingMainSiteTestingFilter filter = new NEAuthenticatingMainSiteTestingFilter();
		AccountValidatorOauthImpl accountValidatorOauthImpl = context.getBean(BEAN_NAME_ACCOUNT_VALIDATOR_OAUTH,
				AccountValidatorOauthImpl.class);
		accountValidatorOauthImpl.setPerfTest(true);
		filter.setAccountValidator(accountValidatorOauthImpl);
	...
	URSOAuthService ursOAuthService() {
		URSOAuthServiceInternalTestingImpl ursOAuthService = new URSOAuthServiceInternalTestingImpl();
		AccountValidatorOauthImpl accountValidatorOauthImpl = context.getBean(BEAN_NAME_ACCOUNT_VALIDATOR_OAUTH,
				AccountValidatorOauthImpl.class);
		accountValidatorOauthImpl.setPerfTest(true);
		ursOAuthService.setAccountValidator(accountValidatorOauthImpl);
		return ursOAuthService;
	}

	@Bean(name = "mainSiteAuthcService")
	MainSiteAuthcService mainSiteAuthcService() {
		MainSiteAuthcServiceImpl mainSiteAuthcService = new MainSiteAuthcServiceImpl();
		mainSiteAuthcService.setRetrieveOauthAccessTokenUrl(env.getProperty(OAUTH_ACCESS_TOKEN_URL));
		mainSiteAuthcService.setContext(this.context);
		return mainSiteAuthcService;
	}
